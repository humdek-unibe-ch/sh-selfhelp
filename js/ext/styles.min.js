...
"use strict";function setKeyboardKeys(){$(document).keydown((function(t){switch(t.keyCode){case 37:$(".book-turnjs").turn("previous");break;case 39:$(".book-turnjs").turn("next")}}))}function updateDepth(t,e){var n=t.turn("page"),s=t.turn("pages"),r=16*Math.min(1,2*n/s);(e=e||n)>3?$(".book-turnjs .p2 .depth").css({width:r,left:20-r}):$(".book-turnjs .p2 .depth").css({width:0}),r=16*Math.min(1,2*(s-n)/s),e<s-3?$(".book-turnjs .secondLast .depth").css({width:r,right:20-r}):$(".book-turnjs .secondLast .depth").css({width:0})}function getViewNumber(t,e){return parseInt((e||t.turn("page"))/2+1,10)}function setSlider(){$("#slider").slider({min:1,max:100,stop:function(){$(".book-turnjs").turn("page",Math.max(1,2*$(this).slider("value")-2))}})}$(document).ready((function(){$(".book-turnjs").each((function(){var t=$(this).data("config");t&&'""'!=t||(t={});var e=!1;t&&t.slider?(e=!0,setSlider()):$("#slider-bar").addClass("d-none"),t.when={turning:function(e,n,s){var r=this;window.location.hash="#page-"+n;var i=$(this),a=i.turn("page"),u=i.turn("pages");if(a>3&&a<u-3){if(1==n)return i.turn("page",2).turn("stop").turn("page",n),void e.preventDefault();if(n==u)return i.turn("page",u-1).turn("stop").turn("page",n),void e.preventDefault()}else if(n>3&&n<u-3){if(1==a)return i.turn("page",2).turn("stop").turn("page",n),void e.preventDefault();if(a==u)return i.turn("page",u-1).turn("stop").turn("page",n),void e.preventDefault()}(updateDepth(i,n),n>=2?$(".book-turnjs .p2").addClass("fixed"):$(".book-turnjs .p2").removeClass("fixed"),n<i.turn("pages")?$(".book-turnjs .secondLast").addClass("fixed"):$(".book-turnjs .secondLast").removeClass("fixed"),t.saveOnTurnPage)&&i.turn("view").forEach((function(t){$(r).find('[page="'+t+'"]').find(":submit").click()}))},turned:function(t,n,s){var r=$(this);2!=n&&3!=n||r.turn("peel","br"),updateDepth(r),e&&$("#slider").slider("value",getViewNumber(r,n)),r.turn("center")},end:function(t,n){var s=$(this);updateDepth(s),setTimeout((function(){e&&$("#slider").slider("value",getViewNumber(s))}),1)},missing:function(t,e){},start:function(t,e,n){}},$(this).turn(t);var n=window.location.hash.substring(1);n&&$(this).turn("page",n.replace("page-","")),e&&$("#slider").slider("option","max",$(this).turn("pages")/2+1),$(this).addClass("animated"),$("#canvas").css({visibility:""})})),setKeyboardKeys()})),$("#canvas").css({visibility:"hidden"});
...
"use strict";function formSubmitEvent(){$("form").on("submit",(function(t){if(1==$(this).find('input[name="ajax"]').val()){var e=1==$(this).find('input[name="is_log"]').val(),n=$(this).find('input[name="redirect_at_end"]').val();t.preventDefault(),$(".alert-danger").remove(),$(".alert-success").remove();var i=[];$(this).find(":submit").each((function(){var t=$(this).html();i.push({btn:this,origLabel:t}),$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"> </span> '+t)}));var r=this;$.ajax({type:"post",url:$(this).attr("action"),data:$(this).serialize(),success:function(t){var a=(new DOMParser).parseFromString(t,"text/html");$(a).find(".alert-success").each((function(){$(this).insertBefore($(r)[0])})),$(a).find(".alert-danger").each((function(){$(this).insertBefore($(r)[0])})),e&&$(r)[0].reset(),i.forEach((function(t){$(t.btn).html(t.origLabel)})),n&&(window.location=n)}})}}))}$(document).ready((function(){formSubmitEvent()}));
...
"use strict";function initSelectImage(e){var t=$(e).attr("id"),a=$(e).attr("class"),c=new IconSelect(t),l=document.getElementById("selectValue-"+t.split("-")[1]);$(e).on("changed",(function(e){l.value=c.getSelectedValue()}));var n=$(e).attr("data-values");n=JSON.parse(n);var i=[],u=$(l).val();n.forEach((function(e){e.value==u?i.unshift({iconFilePath:e.text,iconValue:e.value}):i.push({iconFilePath:e.text,iconValue:e.value})})),c.refresh(i),$(e).addClass(a)}$(document).ready((function(){$(".bootstrapSelect").selectpicker({showTick:!0,allowClear:!0}),$(".selectImage").each((function(e,t){initSelectImage(t)}))}));
...
"use strict";$(document).ready((function(){autosize($("textarea"))}));
