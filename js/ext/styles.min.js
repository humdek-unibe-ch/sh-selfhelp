...
"use strict";function formSubmitEvent(){$("form").on("submit",(function(t){if(1==$(this).find('input[name="ajax"]').val()){var e=1==$(this).find('input[name="is_log"]').val();t.preventDefault(),$(".alert-danger").remove(),$(".alert-success").remove();var r=[];$(this).find(":submit").each((function(){var t=$(this).html();r.push({btn:this,origLabel:t}),$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"> </span> '+t)}));var n=this;$.ajax({type:"post",url:$("form").attr("action"),data:$("form").serialize(),success:function(t){var i=(new DOMParser).parseFromString(t,"text/html");$(i).find(".alert-success").each((function(){$(this).insertBefore($("form"))})),$(i).find(".alert-danger").each((function(){$(this).insertBefore($("form"))})),e&&$(n)[0].reset(),r.forEach((function(t){$(t.btn).html(t.origLabel)}))}})}}))}$(document).ready((function(){formSubmitEvent()}));
...
"use strict";$(document).ready((function(){$("select").selectpicker()}));
...
"use strict";$(document).ready((function(){$(".tabs-container").find(".tab-content").each((function(){$(this).closest(".tabs-container").append($(this))})),$("button.tab-button").click((function(){var t=".tab-content-index-"+$(this).attr("data-context"),s=$(this).closest(".tabs-container").children(t),n=$(this).closest(".tabs-container").children("button.tab-button");$(this).hasClass("active")?($(this).removeClass("active"),s.slideUp("fast")):($(this).addClass("active"),s.slideDown("fast")),n.not($(this)).removeClass("active"),$(this).closest(".tabs-container").children(".tab-content").not(t).hide()}))}));
