...
"use strict";function formSubmitEvent(){$("form").on("submit",(function(t){if(1==$(this).find('input[name="ajax"]').val()){var e=1==$(this).find('input[name="is_log"]').val(),i=$(this).find('input[name="redirect_at_end"]').val();t.preventDefault(),$(".alert-danger").remove(),$(".alert-success").remove();var n=[];$(this).find(":submit").each((function(){var t=$(this).html();n.push({btn:this,origLabel:t}),$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"> </span> '+t)}));var r=this;$.ajax({type:"post",url:$(this).attr("action"),data:$(this).serialize(),success:function(t){var a=(new DOMParser).parseFromString(t,"text/html");check_input_locked_after_submit(),check_radio_locked_after_submit(),check_select_locked_after_submit(),check_slider_locked_after_submit(),check_textarea_locked_after_submit();var s=[];$(r).attr("class").trim().split(" ").forEach((function(t){s.push("."+t)})),$(a).find(s.join(",")).each((function(){$(r).html(this)})),$(a).find(".alert-success").each((function(){$(this).insertBefore($(r)[0])})),$(a).find(".alert-danger").each((function(){$(this).insertBefore($(r)[0])})),e&&$(r)[0].reset(),n.forEach((function(t){$(t.btn).html(t.origLabel)})),i&&(window.location=i)}})}}))}$(document).ready((function(){formSubmitEvent()}));
