...
"use strict";function formSubmitEvent(){$("form").on("submit",(function(t){if(1==$(this).find('input[name="ajax"]').val()){t.preventDefault(),$(".alert-danger").remove(),$(".alert-success").remove();var e=[];$(this).find(":submit").each((function(){var t=$(this).html();e.push({btn:this,origLabel:t}),$(this).html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"> </span> '+t)}));var r=$(this).attr("id");$.ajax({type:"post",url:$("form").attr("action"),data:$("form").serialize(),success:function(t){var n=(new DOMParser).parseFromString(t,"text/html");$(n).find(".alert-success").each((function(){$(this).insertBefore($("form"))})),$(n).find(".alert-danger").each((function(){$(this).insertBefore($("form"))})),$(n).find("#"+r).each((function(){$("#"+r).html($(this).html())})),e.forEach((function(t){$(t.btn).html(t.origLabel)}))}})}}))}$(document).ready((function(){formSubmitEvent()}));
