# v7.10.0
### New Feature
 - **`languagePicker` style**: lets a visitor change the interface language from any page, not only the footer. It renders every language configured in the CMS, so it needs no per-page configuration, and it renders nothing when only one language exists — the same rule the footer follows. Fields: `display_style` (`buttons`, the default, or `select`), `redirect_at_select` (a page keyword to open once a language is chosen; empty reloads the current page, which is what the footer does), and `label`. The behaviour ships as a js include rather than an inline handler because the page CSP pins a script hash, which voids `unsafe-inline` and blocks every inline `<script>` (see `BasePage::getCspRules()`), so an inline picker cannot act on a click. It sets the session language through `AjaxLanguage::ajax_set_user_language`, the endpoint the footer already uses, whose access check is unconditionally true and so works for guests. This makes the language reachable on headless pages and before login — on `login` and `register` a visitor has no profile yet to carry a preference, and a study that hides the site chrome otherwise offers no way to choose at all.
