# v7.9.1 (WIP)
### Bug Fix
 - **Hide site-menu link when a navigation page has no visible children**: If every `navigationContainer` on a nav page fails its condition, the top-level (and dropdown) navbar entry for that page is omitted so users never get a broken `/…/[i:nav]` URL. Same filter applies to the mobile nav JSON. Relies on `NavModel::get_first_nav_section()` / condition visibility. CMS edit routes still show the link. No database migration required.
 - **Hide navigation items whose section condition fails**: `Navigation::fetch_children()` now skips `navigationContainer` sections (and their children) when the section `condition` evaluates to false for the current user, so nested/accordion nav URL lists no longer show links to content that would not render. Uses cached `PageDb::fetch_nav_children` / `fetch_section_fields` (`CACHE_SECTIONS`); condition *results* are not APCu-cached (user/group/time-dependent). CMS edit routes still show all items. `Condition` gains `decode_condition_content()` and `section_is_visible()`. `NavModel::get_first_nav_section()` uses the same visibility rules so the site menu does not deep-link to a hidden first child. Call sites pass the Condition service: `BasePage`, `NestedListModel`, `AccordionListModel`. No database migration required.
 - **Fix select `max` / `data-size` opening far taller than the configured row count**: Long option labels in narrow panels (e.g. CMS properties) wrapped because `.dropdown-menu .text` used `text-wrap: wrap`. bootstrap-select then measured those tall rows as `liHeight` and multiplied by `data-size`, so `max=8` could become a ~640px menu (~18 visual rows). Fix in `server/component/style/select`: keep option text on one line (`nowrap` + ellipsis), pass `data-size` into selectpicker, and re-apply menu `max-height` from real row height on `shown.bs.select`. Rebuilt `css/ext/styles.min.css` and `js/ext/styles.min.js`. No database migration required.
...
# v7.8.2
### Bug Fix
 - **Fix 2FA always failing on production due to a PHP/MySQL timezone mismatch (root cause)**: The 2FA code's `expires_at` was written by PHP (`Login::generate_2fa_code()` via `date()`/`strtotime('+10 minutes')`) while verification compares it against MySQL `NOW()` (`TwoFactorAuthModel::verify_2fa_code()` → `expires_at > NOW()`). The DB connection (`BaseDb`) sets no session `time_zone` and the app never calls `date_default_timezone_set()`, so on a typical Linux production host (PHP in UTC, MySQL in the system/local zone, e.g. UTC+2) the code was born ~2 hours in the past from MySQL's point of view — i.e. already expired — so every verification returned false (`POST …/two-factor-authentication → 302 ?verification_failed=1 → 302 /login`). On Windows dev PHP and MySQL share one timezone, so it worked. Fix: `generate_2fa_code()` now computes `expires_at` with MySQL's own clock via `INSERT … VALUES (…, DATE_ADD(NOW(), INTERVAL <minutes> MINUTE))`, so issuing and checking the code use the same time reference and 2FA is timezone-agnostic. No database migration required. (Recommended hardening: also align the PHP and MySQL timezones server-wide, since other date logic shares this assumption.)
 - **Fix 2FA "Invalid verification code" after the code was already consumed**: A duplicate or concurrent verification request could send the same 6-digit code twice. The first request consumed the code (marked `users_2fa_codes.is_used = TRUE`) and logged the user in, while the second request failed because the code was already used — showing "Invalid verification code" and leaving the user stuck on the 2FA page even though they were effectively authenticated. This was reproducible in production but not locally, where network/proxy/auto-submit timing made the duplicate submit far more likely. Fixes:
   ...
   - `TwoFactorAuthModel::verify_2fa_code()`: clear the `$_SESSION['2fa_user']` marker immediately after a successful verification so an already-consumed code can no longer be re-submitted, and add an `is_logged_in()` wrapper used by the controller.
   - `js/twoFactorAuth.js`: add an init-once guard plus a single-submit guard (`submitFormOnce()`), both stored as flags on the form element (not in a local closure), so the auto-submit triggered by typing the last digit, pasting, or OS/browser OTP autofill can only bind once and POST the code once per page load — even if the init runs more than once.
 - **Note on production-only behavior**: the same-session guard resolves the duplicate-submit case (it relies on PHP serializing requests that share one session). If the real cause is session loss *between* requests, the guard cannot recover the login and the exact infrastructure cause must be confirmed. Things to verify on a production / multi-instance deployment:
   - **Unique `PROJECT_NAME` per instance** — the session cookie is named after `PROJECT_NAME` (`Login::init_session()`); two instances sharing the same name on the same host collide.
   - **Unique PHP `session.save_path` per instance** — otherwise two instances can read/expire each other's session files.
   - **Session cookie scope** — when several instances live under the same domain on different sub-paths, scope the cookie `path` (e.g. to `BASE_PATH`) so cookies don't overwrite each other.
