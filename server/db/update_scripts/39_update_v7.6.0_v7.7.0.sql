...
-- add new field type for selecting data table columns
INSERT IGNORE INTO `fieldType` (`id`, `name`, `position`)
VALUES (NULL, 'select-data_table_columns', 8);

-- add new field `selected_columns`
INSERT IGNORE INTO `fields` (`id`, `name`, `id_type`, `display`)
VALUES (NULL, 'selected_columns', get_field_type_id('select-data_table_columns'), '0');

-- attach field `selected_columns` to style `entryList`
INSERT IGNORE INTO `styles_fields` (`id_styles`, `id_fields`, `default_value`, `help`)
VALUES (
    get_style_id('entryList'),
    get_field_id('selected_columns'),
    '',
    'Select data table columns that should be loaded. If none are selected, all columns are loaded.'
);
