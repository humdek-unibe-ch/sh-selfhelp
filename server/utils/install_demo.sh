...
{
   echo ""
   echo "Usage: $0 -u user_name -n experiment_name -p password -v version"
   echo -e "\t-n Experiment name"
   echo -e "\t-p password"   
   echo -e "\t-u OS user owner of the files"   
   echo -e "\t-v The version of the demo page. Use the form v<maj>_<min>_<rev>"   
   exit 1 # Exit script after printing help
}

while getopts "n:p:u:v:" opt
do
   case "$opt" in
      n ) name="$OPTARG" ;;
      p ) password="$OPTARG" ;;      
...
if [ -z "$password" ]
then
   echo "The password parameter is missing";
   helpFunction
fi
...
cd /home/$user/$name
sudo -u $user git checkout demo_${version//_/.}
...
sudo mysql -D $name < ../db/selfhelp_demo.sql
