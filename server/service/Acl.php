...
class Acl
{
    ...
    public function get_access_levels_db_user_all_pages($id_user)
    {
        ...
        $sql = "SELECT ug.id_users, acl.id_pages, MAX(COALESCE(acl.acl_select, 0)) as acl_select, MAX(COALESCE(acl.acl_insert, 0)) as acl_insert,
                MAX(COALESCE(acl.acl_update, 0)) as acl_update, MAX(COALESCE(acl.acl_delete, 0)) as acl_delete, p.keyword,
                ...
                WHERE ug.id_users = :uid AND acl.id_pages = (CASE WHEN :pid = -1 THEN acl.id_pages ELSE :pid END)
                ...
                FROM acl_users acl
                INNER JOIN pages p ON (acl.id_pages = p.id)
                WHERE acl.id_users = :uid AND acl.id_pages = (CASE WHEN :pid = -1 THEN acl.id_pages ELSE :pid END)
